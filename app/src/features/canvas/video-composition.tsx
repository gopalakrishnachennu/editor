...
export const VideoComposition = () => {
    const { tracks, width, height } = useVideoStore();
            ...
            {tracks.map(track => (
                <div key={track.id} className="absolute inset-0 pointer-events-none">
                    {track.clips.map(clip => {
                        ...
                        // Note: Passing empty object for last arg since we removed 'clips' map from store
                        const volume = getAudioVolume(frame, clip, track, tracks, {});
